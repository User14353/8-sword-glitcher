...
local function RunReanimate()
	...
	if state == "idle" then
			targets.root=cf(Vector3.new(0,1.1+0.2*math.sin(tick()*0.5),0),Vector3.new(-1.9198621771937625,0,3.141592653589793))
			targets.ls=cf(Vector3.new(-1,0.5-0.1*math.sin(tick()*0.5),0),Vector3.new(0.17453292519943295,-1.3962634015954636,0))
			targets.neck=cf(Vector3.new(0,1,0),Vector3.new(-1.3962634015954636-0.03490658503988659*math.sin(tick()*0.5),0,3.141592653589793))
			targets.lh=cf(Vector3.new(-1,-0.6+0.2*math.sin(tick()*0.3),-1.3),Vector3.new(-0.8552113334772214,-1.3962634015954636,0))
			targets.rh=cf(Vector3.new(1,-1,0),Vector3.new(-0.8552113334772214,1.5707963267948966,0))
			targets.rs=cf(Vector3.new(1,0.5-0.1*math.sin(tick()*0.5),0),Vector3.new(0.17453292519943295,1.3962634015954636,0)) 
